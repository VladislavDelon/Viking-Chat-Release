...
const HELP_ANSWERS: [RegExp, string][] = [
  ...
  [
    ...
    'Сообщения шифруются AES-256-GCM прямо на устройстве — хранится только шифротекст.',
  ],
...
export async function seedFor(user: User): Promise<Chat[]> {
  ...
  const posts: [Chat, string, string][] = [
    ...
    [
      news,
      user.id,
      '⚔️ Viking Chat запущен! Сквозное шифрование AES-256-GCM, синхронизация между устройствами, сезонные анимации и северный дизайн — уже здесь.',
    ],
    [news, user.id, '📱 Viking Chat работает на Android и в браузере — входите с любого устройства по ключу аккаунта.'],
  ]
