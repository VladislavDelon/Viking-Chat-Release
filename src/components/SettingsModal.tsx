...
export function SettingsModal({ onClose }: { onClose: () => void }) {
  ...
  async function pickAvatar(f: File | undefined) {
    ...
    try {
      ...
      toast('Фото обновлено', 'Аватар сохранён')
    } catch {
  ...
  async function saveLogin() {
    setBusy(true)
    setLoginErr(null)
    const err = await changeLogin(newLogin, loginPwd)
    setBusy(false)
    if (err) setLoginErr(err)
    else {
      setNewLogin('')
      setLoginPwd('')
      toast('Логин изменён', 'Вход теперь выполняется по новому логину')
    }
  }
      ...
      >
        ...
        <div className="set-section">
          ...
          <div className="set-subtitle">
            <AtSign size={13} /> Логин — для входа в аккаунт: <b className="mono">@{user.login}</b>
          </div>
          <input
          ...
          <button
            ...
            disabled={busy || !newLogin.trim() || !loginPwd}
            onClick={saveLogin}
          >
            Сменить логин
          </button>

          <div className="set-subtitle" style={{ marginTop: 16 }}>
            <KeyRound size={13} /> Пароль
          </div>
          <PasswordInput
            placeholder="Текущий пароль или ключ аккаунта"
            value={curPwd}
            onChange={setCurPwd}
          />
          <PasswordInput
          ...
          <p className="modal-hint">
            Ключ аккаунта нужен для входа на новом устройстве — им расшифровывается переписка.
            Автоматически не скачивается: хранится на устройстве, скачать файлом можно кнопкой выше.
          </p>
        </div>
        ...
        <div className="set-section danger-zone">
          ...
          {!confirmDelete ? (
            ...
            <>
              <p className="modal-hint">
                Аккаунт, все чаты и сообщения будут удалены безвозвратно. Введите пароль
                для подтверждения.
              </p>
              <PasswordInput
                placeholder="Пароль"
